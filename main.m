...
% Plot the quantities
f= figure('color',[39 38 43]./255,'Units','normalized','OuterPosition',[0.0206 0.1481 0.9823 0.7558]);
...
axis equal off; camva(10);c.FontSize = fntSz;c.Location = 'southoutside';c.Color = 'w';
title(sprintf('$$ \\mathbf{v}(\\mathbf{x},%d$$)',t0),'Interpreter','latex','FontSize',fntSz,'color','w'); hold off
...
c = colorbar; axis equal off; camva(10);c.FontSize = fntSz;view(canView1);c.Location = 'southoutside';camlight;c.Color = 'w';
title(sprintf('$$ s_2(\\mathbf{x},%d),\\mathbf{e_2}(\\mathbf{x},%d)$$',t0,t0),'Interpreter','latex','FontSize',fntSz,'color','w'); hold off
xlim([-axLim axLim]); ylim([-axLim axLim]); zlim([-axLim axLim]);
...
quiver3(xct,yct,zct,eig1vert(1,:),eig1vert(2,:),eig1vert(3,:),...
    'k','ShowArrowHead','off','LineWidth',1); hold on; shading interp; 
c = colorbar; axis equal off; camva(10);c.FontSize = fntSz;view(canView1);c.Location = 'southoutside';camlight;c.Color = 'w';
title(sprintf('$$ s_1(\\mathbf{x},%d),\\mathbf{e_1}(\\mathbf{x},%d)$$',t0,t0),'Interpreter','latex','FontSize',fntSz,'color','w'); hold off
xlim([-axLim axLim]); ylim([-axLim axLim]); zlim([-axLim axLim]);
